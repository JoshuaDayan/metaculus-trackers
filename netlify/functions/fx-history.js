// Netlify Function: 1-year FX + DXY history from Yahoo Finance "Close" prices.
...
async function fetchYahooDailyCloses(symbol, startInclusive, endExclusive, { roundDecimals }) {
  ...
  const res = await fetch(url, {
    headers: {
      ...
      "User-Agent": "Mozilla/5.0 (Netlify fx-history)",
    },
...
exports.handler = async (event) => {
  try {
    ...
    const asOfLimit = asOfLimitDate;

    const fetchStart = addDays(asOfLimit, -(WINDOW_DAYS + FETCH_BUFFER_DAYS));
    ...
    // Determine common weekday dates across all series.
    let candidates = null;
    const asOfLimitStr = toISODate(asOfLimit);
    for (const code of Object.keys(TICKERS)) {
      const dates = Object.keys(seriesByCode[code].closeByDate || {}).filter((d) => d <= asOfLimitStr);
    ...
    if (!commonDates.length) throw new Error("No common dates found across series");

    const asOfDate = commonDates.at(-1);

    // Choose baseline date ~1 year before as-of (first common date on/after cutoff).
    const asOfObj = parseISODate(asOfDate);
    if (!asOfObj) throw new Error("Internal date parse failure");
    const cutoffStr = toISODate(addDays(asOfObj, -WINDOW_DAYS));
    const baselineDate = commonDates.find((d) => d >= cutoffStr) || commonDates[0];

    const dates = commonDates.filter((d) => d >= baselineDate && d <= asOfDate);
    if (!dates.length) throw new Error("No dates found in window");

    const baseline = {};
    for (const code of Object.keys(TICKERS)) {
      const v = seriesByCode[code]?.closeByDate?.[baselineDate];
      if (v === undefined) throw new Error(`Missing baseline close for ${code} on ${baselineDate}`);
      baseline[code] = v;
    }

    const series = { dates, close: {}, pct: {} };
    for (const code of Object.keys(TICKERS)) {
      const base = baseline[code];
      const closeArr = dates.map((d) => seriesByCode[code].closeByDate[d] ?? null);
